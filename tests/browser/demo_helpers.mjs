...
async function answerAdaptiveButtonTrials(page, trialCount, chooseIndex = (i) => i % 2) {
  const initialRows = await page.evaluate(() => {
    ...
    return jp.data.get().values().filter((row) =>
      row &&
      typeof row === "object" &&
      row.ado_design &&
  ...
  for (let i = initialRows; i < trialCount; i++) {
    await page.waitForFunction(() => {
      const dataChoiceButtons = Array.from(document.querySelectorAll("button[data-choice]:not([disabled])"));
...
async function answerAdaptiveKeyTrials(page, trialCount, chooseKey = (i) => (i % 2 === 0 ? "b" : "y")) {
  const initialRows = await page.evaluate(() => {
    const jp = window.jsPsych;
    if (!jp || !jp.data) return 0;
    return jp.data.get().values().filter((row) =>
...
async function waitForAdaptiveRows(page, expectedRows, timeout = 240000) {
  return page.waitForFunction((count) => {
    ...
    const eventRows = jp.data.get().values().map((row) => row.value || row);
  ...
  }, { timeout, polling: 250 }, expectedRows);
}
...
async function collectDemoResult(page, expectedRows) {
  return page.waitForFunction((count) => {
    ...
    return {
      ...
      hasChoiceSelectionTime: Object.prototype.hasOwnProperty.call(last, "ado_selection_time_ms"),
  ...
  }, { timeout: 240000, polling: 500 }, expectedRows).then((handle) => handle.jsonValue());
}
