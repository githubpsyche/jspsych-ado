...
try {
  ...
  const ui = await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll(".dd-option-card")).map((card) => card.innerText);
    return {
      cards,
      ...
      hasStyledCard: getComputedStyle(document.querySelector(".dd-option-card")).display.includes("flex"),
    };
  });
  note(ui.cards.length === 2, `renders two SS/LL option cards (got ${ui.cards.length})`);
  note(ui.cards[0].includes("$") && ui.cards[0].includes("available now"), "SS card shows reward and immediate delay");
  note(ui.cards[1].includes("$") && ui.cards[1].includes("available in"), "LL card shows reward and later delay");
  note(ui.contentText.includes("Press S") && ui.contentText.includes("Press L"), "keyboard prompt is visible");
  note(ui.hasStyledCard, "delay-discounting task stylesheet is loaded");

  await page.keyboard.press("l");
  await page.waitForFunction(() => {
    const rows = window.jsPsych.data.get().values().filter((row) =>
      row && row.ado_design && Object.prototype.hasOwnProperty.call(row, "choice")
    );
  ...
  const keyboardChoice = await page.evaluate(() => {
    ...
    return rows[0] ? { response: rows[0].response, choice: rows[0].choice, choiceLabel: rows[0].choice_label } : null;
  });
  note(keyboardChoice && keyboardChoice.response === 1 && keyboardChoice.choice === 1,
