...
} from "../../experiments/delay_discounting/ado/posterior_debug_charts.js";
...
test("formatPosteriorDrawCharts renders every requested posterior parameter", () => {
  const draws = [
  ...
  const output = formatPosteriorDrawCharts(draws, ["k", "tau", "alpha"], { bins: 4, height: 3 });

  assert.match(output, /Posterior draw histograms \(asciichart\):/);
  assert.match(output, /k \(log10\(k\); n=5\)/);
  assert.match(output, /tau \(tau; n=5\)/);
