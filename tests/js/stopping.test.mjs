...
function evalAt(completed, eig, { max_possible_eig = LN2, consecutive_below = 0, ...stopping } = {}) {
...
test("normalizeStoppingConfig fills defaults and the max_trials fallback", () => {
  assert.deepEqual(normalizeStoppingConfig({}, 42), { min_trials: 0, max_trials: 42, eig_fraction: null, consecutive: 1 });
  assert.deepEqual(normalizeStoppingConfig({ min_trials: 8, max_trials: 30, eig_fraction: 0.1, consecutive: 2 }),
...
test("normalizeStoppingConfig rejects junk and turns EIG stopping off for non-positive fraction", () => {
  const n = normalizeStoppingConfig({ min_trials: -3, max_trials: "x", eig_fraction: 0, consecutive: 0 }, null);
...
test("threshold is a fraction of max EIG, so it scales with response cardinality", () => {
  ...
  const cat = evalAt(10, 0.09, { min_trials: 8, max_trials: 42, eig_fraction: 0.1, max_possible_eig: LN3 });
  assert.equal(cat.should_stop, true);
  assert.equal(cat.stop_reason, "eig_fraction");

  const bin = evalAt(10, 0.09, { min_trials: 8, max_trials: 42, eig_fraction: 0.1, max_possible_eig: LN2 });
...
test("de-bounce: consecutive=2 needs two sub-threshold refits in a row; a rebound resets the streak", () => {
  const cfg = normalizeStoppingConfig({ min_trials: 8, max_trials: 42, eig_fraction: 0.1, consecutive: 2 });
  // first sub-threshold refit: streak -> 1, no stop yet
  const r1 = evaluateStopping({ completed_trials: 10, eig: 0.05, max_possible_eig: LN2, consecutive_below: 0, stopping: cfg });
  assert.equal(r1.should_stop, false);
  assert.equal(r1.consecutive_below, 1);
  // EIG rebounds above threshold: streak resets
  const r2 = evaluateStopping({ completed_trials: 11, eig: 0.2, max_possible_eig: LN2, consecutive_below: r1.consecutive_below, stopping: cfg });
  assert.equal(r2.should_stop, false);
  assert.equal(r2.consecutive_below, 0);
  // two sub-threshold refits in a row -> stop
  const r3 = evaluateStopping({ completed_trials: 12, eig: 0.04, max_possible_eig: LN2, consecutive_below: 0, stopping: cfg });
  const r4 = evaluateStopping({ completed_trials: 13, eig: 0.03, max_possible_eig: LN2, consecutive_below: r3.consecutive_below, stopping: cfg });
...
test("a higher eig_fraction stops earlier on the same decreasing EIG trajectory", () => {
  ...
  function firstStop(fraction) {
    ...
    for (let i = 0; i < trajectory.length; i++) {
      const r = evaluateStopping({ completed_trials: i + 1, eig: trajectory[i], max_possible_eig: LN2, consecutive_below: streak, stopping: cfg });
  ...
  assert.ok(strict < lenient, `stricter (higher) fraction should stop earlier: strict=${strict} lenient=${lenient}`);
});
