...
//
// Like tests/js/stan_recovery.smoke.mjs, this is NOT part of `node --test`: it loads
// the BROWSER wasm (compiled -sENVIRONMENT=web,worker) in Node by shimming `fetch`
// for file: URLs, and bypasses the Web Worker. It exercises weber_dot_comparison.stan,
// the weber_dots adapter (choiceProbLL/buildData), summarizeDraws, MI design
...
// Curated numerosity pairs spanning easy -> hard ratios (the array escape hatch).
const PAIRS = [
...
function runRecovery(trueW, seed, nTrials) {
  ...
  let { design } = selectOptimalDesign(designs, samplePriorDraws(weber.prior, 2000, prior_rng), weber.choiceProbLL);
  const trials = [];
  ...
  for (let t = 0; t < nTrials; t++) {
    ...
    ({ design } = selectOptimalDesign(designs, draws, weber.choiceProbLL));
  }
  return summary;
}
